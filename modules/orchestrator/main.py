...
def seller_agent_chat(req: SellerChatRequest):
    ...
    # Build active catalog context (scoped to store-owned SKUs)
    m_data = DEMO_MERCHANT_CATALOG.get(req.merchant_id, {})
    prods = m_data.get("products", {})
    cat_lines = []
    for pid, p in prods.items():
        ...
        cat_lines.append(f"• [{pid}] {p.get('name')} ({p.get('category')}): Selling Price ₹{price_inr:.2f} | Wholesale Cost ₹{cost_inr:.2f} | Current Margin: {margin_pct}% | Stock: {p.get('stock', 30)}")
    catalog_context = "\n".join(cat_lines[:25])
